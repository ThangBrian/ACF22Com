...
extern krait_intg transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(void) {
	...
	krait_char c;	/* input symbol */
	krait_intg state = 0;		/* initial state of the FSM */
	krait_intg lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	krait_intg lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	krait_intg lexLength;		/* token length */
	krait_intg i;				/* counter */
	krait_char newc;			/* new char */
	...
	while (1) { /* endless loop broken by token returns it will generate a warning */
		...
		*/
		...
		case '>':
			if (readerGetChar(sourceBuffer) == '=') {
				...
				readerRetract(sourceBuffer);
				currentToken.code = REL_OP_T;
				currentToken.attribute.assignmentOperator = OP_GT;
			}
			return currentToken;
		case '<':
			if (readerGetChar(sourceBuffer) == '=') {
		...
		*/
		...
		default: // general case
			...
			lexemeBuffer = readerCreate((krait_intg)lexLength + 2, 0, MODE_FIXED);
			if (!lexemeBuffer) {
...
krait_intg nextState(krait_intg state, krait_char c) {
	krait_intg col;
	krait_intg next;
	col = nextClass(c);
...
krait_intg nextClass(krait_char c) {
	krait_intg val = -1;
	switch (c) {
	case CHRCOL2:
		val = 2;
...
Token funcIL(krait_char lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tlong >= 0 && tlong <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (krait_intg)tlong;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
...
Token funcID(krait_char lexeme[]) {
	...
	//krait_char lastch = lexeme[length-1];
	krait_intg isID = KRAIT_FALSE;
...
Token funcSL(krait_char lexeme[]) {
	Token currentToken = { 0 };
	krait_intg i = 0, len = (krait_intg)strlen(lexeme);
...
krait_void printToken(Token t) {
	extern krait_char* keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
	...
	case UNS_T:
		...
		break;
	case STR_T:
		printf("STR_T\t\t%d\t ", (krait_intg)t.attribute.codeType);
		printf("%s\n", readerGetContent(stringLiteralTable, (krait_intg)t.attribute.codeType));
		break;
	case LPR_T:
		printf("LPR_T\n");
		break;
...
Token funcDL(krait_char lexeme[]) {
	Token currentToken = { 0 };
	krait_float float1;
	if (lexeme[0] != '\0' && strlen(lexeme) > DBL_LEN) {
		currentToken = (*finalStateTable[ESNR])(lexeme);
	}
	else {
		float1 = (krait_float)atof(lexeme);
		if (float1 != 0) {
			currentToken.code = DBL_T;
			currentToken.attribute.floatValue = float1;
		}
		else {
			currentToken = (*finalStateTable[ESNR])(lexeme);
		}
