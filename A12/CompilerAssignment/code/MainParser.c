...
krait_intg errorNumber;     /* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern krait_intg syntaxErrorNumber /* number of syntax errors reported by the parser */;
extern krait_intg line; /* source code line number - defined in scanner.c */
...
extern krait_void startParser(krait_void);
extern krait_intg startScanner(ReaderPointer sc_buf);

static krait_void printParserError(krait_char* fmt, ...);
static krait_void displayParser(ReaderPointer ptrBuffer);
static krait_long getParserFileSize(krait_char* fname);
static krait_void callGarbageCollector(krait_void);
...
krait_void printParserError(krait_char* fmt, ...) {
...
krait_long getParserFileSize(krait_char* fname) {
	FILE* input;
	krait_long flength;
	input = fopen(fname, "r");
	if (input == NULL) {
...
krait_void displayParser(ReaderPointer ptrBuffer) {
...
krait_void callGarbageCollector(krait_void) {
